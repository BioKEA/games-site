...
// from the bundled artifact under public/<slug>/, if present).
function readGames() {
...
function injectBackButton(indexHtmlPath) {
  ...
  const next = html.replace(
    /<body([^>]*)>/i,
    (match, attrs) => `<body${attrs}>${BACK_BUTTON_HTML}`,
  );
...
function subscribeLinkHtml(slug) {
  const href = `https://biokea.ai/subscribe?source=${encodeURIComponent(slug)}`;
...
// /<slug>/ traffic is tracked under the same G-WYL7J2D7SG property as the
// rest of games.biokea.ai. Prefers <head>; falls back to after <body> for
// any game whose index.html lacks a head. Idempotent.
...
if (!token) {
  console.warn(
    '[games-build] No GITHUB_TOKEN and no gh auth — skipping clones. /<slug>/ routes will 404 unless public/<slug>/ is already populated locally.',
  );
  process.exit(0);
}
...
for (const game of games) {
  ...
  try {
    ...
    run(`npx vite build --base /${game.slug}/`, {
      cwd: work,
      env: { ...gitEnv, ...buildEnv },
    });
    ...
    console.error(
      `[games-build] ${game.slug}: bundled fallback under public/${game.slug}/ remains in place.`,
    );
    failCount++;
  } finally {
